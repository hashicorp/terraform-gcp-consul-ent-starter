...
resource "tls_self_signed_cert" "ca" {
  ...
  #   command = "echo '${tls_self_signed_cert.ca.cert_pem}' > ./consul-ca.pem"
  # }
}

# Generate another private key. This one will be used
# To create the certs on your consul nodes
resource "tls_private_key" "server" {
  algorithm = "RSA"
  rsa_bits  = 2048

  # provisioner "local-exec" {
  #   command = "echo '${tls_private_key.server.private_key_pem}' > ./consul-key.pem"
  # }
}
...
resource "tls_locally_signed_cert" "server" {
  ...
  # provisioner "local-exec" {
  #   command = "echo '${tls_locally_signed_cert.server.cert_pem}' > ./consul-crt.pem"
  # }
}
